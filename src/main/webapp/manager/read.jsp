...
<DIV class='content'>
  ...
  <DIV class='title_line'>관리자 정보 조회</DIV><br>
     ...
     <input type = "hidden" name="managerno" value="${managerVO.managerno}">
        ...
        <ul>
          <li class="li_none" style='font-size:18px;'>
            ...
            <DIV><A> 관리자 번호 : </A>${managerVO.managerno }</DIV>
            <DIV><A> 아이디 : </A>${managerVO.id }</DIV>
            <DIV><A> 이름 : </A>${managerVO.mname }</DIV>

            <span>
              <c:if test="${managerVO.sizes > 0}">
                사진 : <A href='${pageContext.request.contextPath}/download?dir=/manager/storage&filename=${managerVO.files}'>${managerVO.files}</A>
              </c:if>
            </span>
          </li>
          <li class="li_none">    
            <div id='file1Panel'>
              ...
              <c:choose>
                <c:when test="${fn:endsWith(files, '.jpg')}">
                  <IMG id='files' src='./storage/${managerVO.files}'  style='width:30%;'>
                </c:when>
                <c:when test="${fn:endsWith(files, '.png')}">
                  <IMG id='files'  src='./storage/${managerVO.files}' >
                </c:when>
              </c:choose>
            </div>
          </li>
          <li class="li_none" style='font-size:18px;'>
            ...
            <DIV><A> 전화번호 :  </A>${managerVO.tel }</DIV>
            <DIV><A> 우편번호 : </A>${managerVO.zipcode }</DIV>
            <DIV><A> 주소 :  </A>${managerVO.address1 }</DIV>
            <DIV><A> 상세 주소 :  </A>${managerVO.address2 }</DIV>
             <button type="button" onclick="location.href='./update.do?managerno='+${managerVO.managerno}" class="btn btn-primary btn-md" style='font-size:15px; float:right;'>수정하러 가기!</button>       
          </li>
        </ul>
